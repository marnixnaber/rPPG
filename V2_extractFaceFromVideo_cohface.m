function [pixelValPerFrame,faceMap,vidInfo,faceDetection,faceTracking, sbjName] = extractFaceFromVideo(fileName,TOI, showSel, sbjName)
...
for f = 1:round(vidInfo.nFrames_sel) % loop through each image frame to calculate average pixel value in region of interest
    ...
    if faceDetection.nFacesDetected > 0 & faceTracking.active
        ...
        if showSel || f == round(vidInfo.nFrames_sel) % l
    ...
    if showSel & faceDetection.nFacesDetected > 0 & faceTracking.active | f == round(vidInfo.nFrames_sel) % l
        textString = {
    ...
    if faceDetection.warningMess > 0 & showSel || faceDetection.warningMess > 0 && f ==round(vidInfo.nFrames_sel) % l
