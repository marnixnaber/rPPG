function [pixelValPerFrame,faceMap,vidInfo,faceDetection,faceTracking, sbjName] = v2_extractFaceFromVideo(fileName,TOI, ...
    makeFaceVid, showVid, sbjName, expPlotsFolder)
...
for f = 1:round(vidInfo.nFrames_sel) % loop through each image frame to calculate average pixel value in region of interest
    ...
    if faceDetection.nFacesDetected > 0 & faceTracking.active
        ...
        if makeFaceVid || f == round(vidInfo.nFrames_sel) % l
    ...
    if makeFaceVid & faceDetection.nFacesDetected > 0 & faceTracking.active | f == round(vidInfo.nFrames_sel) % l
        textString = {
    ...
    if faceDetection.warningMess > 0 & makeFaceVid || faceDetection.warningMess > 0 && f ==round(vidInfo.nFrames_sel) % l
    ...
    %% show video frame
    if makeFaceVid
        imToVid(f).thisImage = ImDisp;
    end
    
    if makeFaceVid & showVid & faceDetection.nFacesDetected > 0 & faceTracking.active || f == round(vidInfo.nFrames_sel) % l
        figure(1);
        imshow(ImDisp)
        drawnow;
        clear ImDisp
    ...
    if f == round(vidInfo.nFrames_sel)
        if ~(exist('plots'))
            mkdir('plots');
        end
        
        cd(['plots' filesep expPlotsFolder])
...
if makeFaceVid
    ...
    refFrameWidth = max(allFrameWidth);
    ...
    for thisFrame = 1:length(imToVid)
        ...
        if size(imToVid(thisFrame).thisImage, 2) <= refFrameWidth
            ...
            thisFrameWidth = size(imToVid(thisFrame).thisImage, 2);
            widthDiff = refFrameWidth - thisFrameWidth;
            %         rowToStartPadding = size(imToVid(thisFrame).thisImage, 2) - widthDifference-1;
            
            imToVid(thisFrame).thisImage(:,thisFrameWidth:refFrameWidth, :) = 0;
